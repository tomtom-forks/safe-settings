...
`
  ...
  async updateRepos (repo) {
    ...
    // IMPORTANT: clone (don't mutate) `this.config.repository` and
    // `subOrgConfig.repository`. They are shared across all repositories in
    // the same Settings instance. When `eachRepositoryRepos` processes repos
    // in parallel via `Promise.all(...map(...))`, mutating the shared object
    // here lets the `name` field leak between concurrent iterations and
    // results in the Repository plugin sending a `PATCH /repos/{owner}/{repo}`
    // with the wrong `name` -- which GitHub interprets as a repo rename.
    // See docs/rebase-2.1.18/CHANGELOG.md (post-rebase fixes) for details.
    ...
    if (subOrgConfig) {
      ...
      if (suborgRepoConfig) {
        // Same shared-mutable-state hazard as above -- clone before assigning.
  ...
  storeSubOrgConfigIfNoConflicts (subOrgConfigs, overridePath, repoName, data) {
    ...
    if (existingConfigForRepo && existingConfigForRepo.source !== overridePath) {
      // TomTom fork (2.1.18-tomtom-rc.5+): match 0.3.3 behavior — last-loaded
      // suborg overwrites earlier (alphabetical file ordering). Upstream 2.1.18
      // throws here, which freezes the entire org sync until the conflict is
      // resolved in the admin repo. That's safer in principle but operationally
      // unacceptable for prod: a single misconfigured suborg file from any team
      // blocks reconciliation for *every* repo.
      //
      // To find all currently-conflicting repos:
      //   kubectl logs deploy/safe-settings-deployment | grep SUBORG_CONFLICT
      //
      // Long-term mitigation: a PR-time validator in the admin repo blocks
      // conflicting configs before they merge. See
      // docs/rebase-2.1.18/NEXT-SESSION-PROD-DAAS.md.
      this.log.warn(`SUBORG_CONFLICT repo=${repoName} winner=${overridePath} loser=${existingConfigForRepo.source}`)
    }

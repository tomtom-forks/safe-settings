...
module.exports = class Repository extends Plugin {
  ...
  updateSecurity (repoData, resArray) {
    if (this.security?.enableVulnerabilityAlerts === true || this.security?.enableVulnerabilityAlerts === false) {
      this.debug.log(`Found repo with security settings ${JSON.stringify(this.security)}`)
      if (this.security.enableVulnerabilityAlerts === true) {
        this.debug.log(`Enabling Dependabot alerts for owner: ${repoData.owner.login} and repo ${repoData.name}`)
        if (this.nop) {
        ...
        this.debug.log(`Disabling Dependabot alerts for for owner: ${repoData.owner.login} and repo ${repoData.name}`)
        if (this.nop) {
      ...
      this.debug.log(`no need to update security for ${repoData.name}`)
      if (this.nop) {

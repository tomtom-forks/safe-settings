...
module.exports = class Branches extends Plugin {
          ...
          .map(branch => {
            ...
            if (this.isEmpty(branch.protection)) {
              ...
              return this.github.repos.getBranchProtection(params).then((result) => {
                ...
                const currentBranchProtection = this.reformatAndReturnBranchProtection(result.data)

                this.useCurrentBranchProtectionIfNotOverriddenOrDisable(currentBranchProtection, branch.protection)

                const changes = mergeDeep.compareDeep(currentBranchProtection, branch.protection)
   ...
   */
  useCurrentBranchProtectionIfNotOverriddenOrDisable (currentBranchProtection, overriddenBranchProtection) {
    this.log.debug('Branch protection current: %o', currentBranchProtection)
    this.log.debug('Branch protection overridden: %o', overriddenBranchProtection)
    ...
    if (overriddenBranchProtection?.required_status_checks === undefined) {
      ...
      if (currentBranchProtection.required_status_checks) {
        ...
        this.log.debug('Enabling required_status_checks: %o', overriddenBranchProtection.required_status_checks)
      }
      else {
    ...
    // else if (overriddenBranchProtection?.required_status_checks &&
    //   overriddenBranchProtection?.required_status_checks.strict &&
    //   overriddenBranchProtection?.required_status_checks.checks === undefined) {
    //     this.log.debug('Only required_status_checks.strict is set: %o', overriddenBranchProtection?.required_status_checks)
    //     if (currentBranchProtection.required_status_checks && currentBranchProtection.required_status_checks.checks) {
    //       this.log.debug('Using required_status_checks.checks from UI: %o', currentBranchProtection.required_status_checks)
    //       const checks = currentBranchProtection.required_status_checks.checks
    //       overriddenBranchProtection.required_status_checks.checks = checks
    //     }
    // }

...
module.exports = class Repository extends ErrorStash {
  constructor (nop, github, repo, settings, installationId, log, errors) {
    ...
    this.github = github
    this.settings = { mediaType: { previews: ['nebula-preview'] }, ...settings, ...repo }
    ...
    return this.github.repos.get(this.repo)
      .then(resp => {
        ...
        if (e.status === 404) {
          ...
          this.logError(`Error while syncing repository: ${JSON.stringify(e)}`)
        }
      })
  }
  ...
  updateDefaultBranch (oldname, newname, resArray) {
    ...
    return this.github.repos.getBranch({
      ...
      if (e.status === 404) {
        ...
        this.logError(`Error while updating default branch: ${JSON.stringify(e)}`)
      }
    })
  }

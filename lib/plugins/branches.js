...
module.exports = class Branches extends ErrorStash {
          ...
          .map(branch => {
            ...
            if (this.isEmpty(branch.protection)) {
              ...
              if (branch.name === 'default') {
                ...
                this.log.info(`Deleting default branch protection for branch ${currentRepo.data.default_branch}`)
              }
              ...
              return this.github.repos.getBranchProtection(params).then((result) => {
                ...
                this.useCurrentBranchProtectionIfNotOverriddenOrDisable(this.reformatAndReturnBranchProtection(result.data), params)
                ...
                if (e.status === 404) {
                  ...
                  this.log.debug(`Adding branch protection ${JSON.stringify(params)}`)
                  return this.github.repos.updateBranchProtection(params).then(res => this.log.info(`Branch protection applied successfully ${JSON.stringify(res.url)}`)).catch(e => { this.logError(`Error applying branch protection ${JSON.stringify(e)}`); return [] })
                } else {
                  this.logError(e)
                  if (this.nop) {
   ...
   */
    useCurrentBranchProtectionIfNotOverriddenOrDisable (currentBranchProtection, overriddenBranchProtection) {
      ...
      const supportedAttributes = [{"name": "enforce_admins", "required": true},
                                   {"name": "required_linear_history", "required": false},
                                   {"name": "restrictions", "required": true}
                                  ]
      supportedAttributes.forEach((e) => {
        this.checkAttribute(currentBranchProtection, overriddenBranchProtection, e.name, e.required)
      })

      this.log.debug('Branch protection updated: %o', overriddenBranchProtection)
    }

    checkAttribute(currentBranchProtection, overriddenBranchProtection, key, required = false) {
      ...
      if (overriddenBranchProtection[key] === undefined) {
        ...
        if (currentBranchProtection.hasOwnProperty(key)) {
          overriddenBranchProtection[key] = currentBranchProtection[key]
          this.log.debug('Setting %s to %o from UI', key, overriddenBranchProtection[key])
        }
        else {
          if (required) {
